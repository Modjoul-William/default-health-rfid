...
def lambda_handler(event,context):
    ...
    def get_ClearStream_JSON_data(BEARER_TOKEN):
        ...
        # ----------------------------------------------------------------------------------------------------------------------
        # 
        # Commenting DVA3 out for now as these readers are no longer important
        # 
        ...
        # ----------------------------------------------------------------------------------------------------------------------
            ...
            # These are related to Coles sites
        for device in devices:
            if device['online'] == "" and device['name'].endswith('ESDC'):
                coles_offline_devices.append(device['name'])

        # Check to see if devices for amazon beta sites are down
        for device in devices:
            if device['online'] == "" and device['name'].endswith(('DCL5','DDP9','DLT3','DKY8','DAX5', 'DVA3')):
                if not device['name'].startswith('OLD'):
                    offline_devices.append(device['name'])
